...
const ActivityPage: React.FC = () => {
  ...
  const handleGhView = useCallback((val: "graph" | "list" | null) => {
    ...
    <PullToRefresh onRefresh={handleRefresh}>
      ...
      >
        ...
        <Typography variant="body1" sx={{ opacity: 0.75, mb: 4 }}>
          A unified timeline of my open-source work. npm publishes appear first,
          with a commit‑style graph of GitHub repository activity.
        </Typography>
        ...
        <Typography variant="body2" sx={{ mt: 4, opacity: 0.7 }}>
          GitHub and npm sections use local caching with background refresh to
          reduce API calls while keeping activity reasonably fresh. (
          {githubEvents.length} GitHub events / {npmPackages.length} packages)
        </Typography>
