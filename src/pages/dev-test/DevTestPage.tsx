...
const DevTestPage: React.FC = () => {
  ...
  const testProjects = [
    {
      id: "test-originally-featured",
      title: "Test Project (Originally Featured)",
      originalFeatured: true,
    },
    {
      id: "test-originally-not-featured",
      title: "Test Project (Originally Not Featured)",
      originalFeatured: false,
    },
    {
      id: "test-neutral-project",
      title: "Test Neutral Project",
      originalFeatured: false,
    },
  ];
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Alert severity={isDevelopment ? "success" : "warning"} sx={{ mb: 3 }}>
        <Typography variant="body1">
          <strong>Current Environment:</strong> {currentMode}
          <br />
          <strong>Can Modify Stars:</strong> {isDevelopment ? "Yes" : "No"}
          <br />
          {!isDevelopment && (
            <>
              Star modifications are disabled in production mode. Stars can only
              be toggled in development.
            </>
          )}
        </Typography>
      </Alert>
              ...
              <CardContent>
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                    <IconButton
                      onClick={() =>
                        toggleStar(project.id, project.originalFeatured)
                      }
                      size="large"
                      sx={{
        ...
        <Card variant="outlined">
          <CardContent>
            ...
            >
              {JSON.stringify(
                JSON.parse(
                  localStorage.getItem("featured-status-overrides") || "{}"
                ),
                null,
                2
              )}
      ...
      <Box sx={{ mt: 4 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          1. In development mode, click the star buttons to toggle starred
          status
          <br />
          2. Check the localStorage section above to see overrides being saved
          <br />
          3. Build and preview the app (
          <code>npm run build && npm run preview</code>) to test production mode
          <br />
          4. In production mode, star buttons should be disabled and show
          console warnings
        </Typography>
      </Box>
    </Container>
