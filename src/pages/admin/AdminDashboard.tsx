...
const AdminDashboard: React.FC = () => {
  ...
  const loadDashboardData = async (days: number) => {
    ...
    try {
      ...
      if (
        errorMessage.includes("Firestore not initialized") ||
        errorMessage.includes("Firebase configuration incomplete")
      ) {
        console.log(
          "[AdminDashboard] Transforming Firebase error to user-friendly message"
        );
        errorMessage =
          "Firebase is not configured. Please add your Firebase credentials to the .env file. See FIREBASE_QUICKSTART.md for setup instructions.";
      }
  ...
  const pageViewsData: ChartData[] =
    metrics?.pageViewsTrend.map((item) => ({
      ...
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        <TabPanel value={activeTab} index={0}>
          ...
          {!loading &&
            !error &&
            metrics &&
            metrics.overview.pageViews === 0 && (
              <Alert severity="info" sx={{ mb: 4 }}>
                <AlertTitle>No Data Available Yet</AlertTitle>
                Start browsing the portfolio to generate analytics. Page views,
                project interactions, and other events will appear here.
              </Alert>
            )}
